...
public class AppExceptionsHandler {
	...
	public ResponseEntity<Object> handlePersistenceException(DataIntegrityViolationException ex) {
		...
		if (cause instanceof SQLIntegrityConstraintViolationException
			...
			apiError = new ApiError(UUID.randomUUID().toString(), LocalDateTime.now(),
					ApiErrorCodeEnum.INTERNAL_SERVER_ERROR.name(),
					ApiErrorMessageKeyEnum.DATABASE_CONSTRAINT_VIOLATION.name());
		}
		logger.info("xxxxxxxxx   Exception : : : " + ex.getMessage());
	...
	private ApiError SQLIntegrityConstraintViolation(String exceptionMessage) {
	    String message = null;
	    String code = null;
	    if (exceptionMessage.contains("Duplicate entry")) { // Unique Key Constraint Violation
	        message = getConstraintKeyFromExceptionMessage("for key '", exceptionMessage);
	        code = ApiErrorCodeEnum.UNIQUE_KEY_CONSTRAINT_VIOLATION.name();
	    } else if (exceptionMessage.contains("cannot be null")) { // Not Null Constraint Violation
	        message = getConstraintKeyFromExceptionMessage("`", exceptionMessage);
	        code = ApiErrorCodeEnum.INTERNAL_SERVER_ERROR.name();
	    } else if (exceptionMessage.contains("foreign key constraint fails")) { // Foreign Key Constraint Violation
	        message = getConstraintKeyFromExceptionMessage("FOREIGN KEY (`", exceptionMessage);
